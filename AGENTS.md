...
- `docs/ai/` — **`docs/ai/mapa-ejecucion-brox.md` is the one to open first**: the cover page of progress (stages E0…E9, what closed, what's next). What is still missing to close the current stage lives in `docs/ai/checklist-captura-moat-e-inteligencia-inmobiliaria.md`. The migration-era docs (`checklist-migracion.md`, `plan-maestro-…`, `plan-migracion-*`, the Blazor/Java inventories) carry a `HISTÓRICO — NO GOBIERNA EL ROADMAP ACTUAL` banner and use a **different** E1…E5 lettering than the current map. `docs/ai/matriz-operacion-rol.md` is the source of truth for **who can call what and where scope is decided** — read it before building any role-aware screen; it is enforced by a test. `docs/ai/seguridad-no-leer.md` is named "do not read" by convention — leave it alone.
