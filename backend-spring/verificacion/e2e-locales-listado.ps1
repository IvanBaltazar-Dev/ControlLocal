...
"@ | Out-Null
    ...
    if ($idOrganizacionOtra) {
        ...
        # V88: el alta por la API deja su fila de responsable y la FK bloquea el borrado.
        Sql "delete from asignacion_responsable_propiedad where id_propiedad in (select id_propiedad from propiedad where codigo like '$prefijo-%')" | Out-Null
