...
| GET | `/propiedades/{id}/responsable/historial` | BROKER, TENANT_ADMIN | **El expediente de traspasos es superficie de GOBIERNO** (C2), del más reciente al más antiguo. Lo lee el BROKER **dentro de su alcance de supervisión** —la propiedad tiene que responder ante uno de sus agentes, comprobado en el servicio con `alcances.alcanza`— y el TENANT_ADMIN dentro de su tenant. **El AGENTE no, incluido el responsable vigente**: sabe que responde él y tiene lo que necesita para operar, pero **no hereda** responsables anteriores, motivos de traspaso ni observaciones de gobierno sobre agentes que ya no la llevan; el texto libre del motivo es dato interno de gobierno. Una propiedad **FALTANTE** no responde ante nadie, así que ningún broker la alcanza: es gobierno del tenant hasta que se asigne. Frontera de tenant **antes** que el rol: un id de otra corredora responde **404**, no 403. Si el nuevo responsable necesita contexto, la respuesta no es abrirle el expediente sino una **nota de traspaso operativa**, que no existe todavía. | E4 |
