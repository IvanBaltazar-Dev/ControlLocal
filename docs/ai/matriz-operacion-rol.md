...
> su historial). Se cuenta con este comando, sobre este archivo:
>
> ```sh
> grep -cE '^\| (GET|POST|PUT|DELETE|PATCH) \|' docs/ai/matriz-operacion-rol.md
> ```
>
> **Las barras van escapadas, y no es un detalle de estilo.** El comando que estaba escrito aquí
...
> fichero. Devolvía **345** —el total de líneas, medido el 2026-08-30— en vez de **189**. Un
> comando de verificación que no verifica es peor que ninguno: da un número, y por eso nadie lo
> vuelve a mirar.
>
> `MatrizOperacionRolTest` compara la tabla con los controladores fila a fila, pero **no lee este
> número**: por eso pudo envejecer siete operaciones sin que nada se pusiera rojo.
...
| PUT | `/indicadores/metas` | BROKER | Fija o revisa metas; idempotente por (agente, KPI, mes) y **con motivo obligatorio**, que queda en la serie. **El ADMIN no entra**: administrar usuarios no es dirigir producción, y un administrador que además dirija comercialmente lo hace con su rol de broker. No existe meta de equipo: es la suma de las de sus agentes (D-E2-2 §5). Fijar la de un agente fuera del alcance es regla de negocio, no 403. | E2.6 |
| POST | `/indicadores/metas/propuestas` | AGENTE | El agente **propone** un ajuste de su propia meta, con motivo. No la cambia: queda en espera. Una sola propuesta viva por KPI y mes; la siguiente reemplaza a la anterior. | E2.6 |
| GET | `/indicadores/metas/propuestas` | BROKER, TENANT_ADMIN | Lo que espera decisión sobre los agentes del alcance. El ADMIN lee porque gobierno sin visibilidad no puede responder por qué un equipo está en rojo. | E2.6 |
| POST | `/indicadores/metas/propuestas/{id}/decision` | BROKER | Acepta o rechaza. **Motivo obligatorio en los dos casos**: un «no» sin explicación deja al agente sin nada que aprender y un hueco en el historial. Decidir sobre un agente fuera del alcance es regla de negocio, no 403. | E2.6 |
