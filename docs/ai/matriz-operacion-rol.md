...
| GET | `/captura/definicion` | TODOS | Qué campos aplican a un `tipoPropiedad` + una o dos `operaciones`, en tres familias: comunes, del tipo y **un bloque económico por encargo**. Con `VENTA,ALQUILER` la ficha física viene una vez y la condición económica dos. Sin alcance porque no devuelve datos de nadie — devuelve la **definición**, que sale del catálogo del tenant (`catalogo_atributo`). Existe para que ni Angular ni KAIROS tengan su propia matriz «tipo → campos». | E4 |
