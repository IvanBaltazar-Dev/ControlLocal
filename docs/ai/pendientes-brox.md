...
> que `ManejadorErroresApi.reglaNegocio` mapea a **400 BAD_REQUEST**:
>
> ```
> PUT /propiedades/1
...
> sobre el `PUT`. Para un valor **escalar** el alta hace lo mismo —`escribirAlAlta`
> llama a `definicionDe` antes de tocar nada, igual que `escribirEnEdicion`—, pero
> **no se disparó un `POST /propiedades`**: habría creado una propiedad en
> `controllocal_dev`, y esta medición se hizo sin escribir nada.
>
> **Y no vale para el MULTIVALOR**, medido el 2026-08-26 leyendo el código:
> `escribirAlAlta` y `escribirEnEdicion` comprueban `esMultivalor()` y delegan en
> `escribirMultivalor` **antes** de llamar a `definicionDe`, y `escribirMultivalor`
> consulta el catálogo **sólo si el ancla no existe**. La excepción está registrada
> como deuda propia en **§2.3 sexies · N22** y **no se cierra en 5A**, por decisión
> de CONTROL.

- **Ruta**: `backend-spring/controllocal-web/src/main/java/com/controllocal/web/http/ManejadorErroresApi.java`
  (`unicidadViolada` y `mensajeDuplicado`).
...
- **Lo que sí está probado en 5A**, y lo que no: que el rechazo de una clave
  ...
  al cliente. Barrido el 2026-08-26: **ninguna prueba Java afirma un código
  HTTP** — 0 apariciones de `isBadRequest`, `isConflict`, `HttpStatus.`,
  `status().is` ni `getStatusCode` en **todos** los `src/test` de
  `backend-spring`. Lo que fija un 400 en esta familia es la comprobación
  `una clave fuera del catalogo se rechaza` de `e2e-editor-universal.ps1`, y lo
  hace sobre `zzz_clave_inexistente`, una clave que **nunca existió**, no sobre
  una **retirada** ni sobre un valor **fuera de vocabulario**. **Los dos casos de
  esta deuda están sin fijar.** Se anota aquí,
...
| **V84 · el fixture** | **Un comentario de `V84` sigue afirmando que «un fixture las escribe en cada corrida»**, que es falso desde este mismo corte (N2) | `V84__la_ocupacion_y_los_servicios_con_vocabulario.sql`, comentario de la sección del reparto («EL TAMANO DEL LEGADO SE ESCRIBE COMO INVARIANTE») | **`V84` está aplicada en las dos bases**: editar el fichero invalida el checksum y obliga a repetir el ciclo entero de H1. La regla del repositorio es *never edit an applied migration*, y una migración aplicada es además **evidencia fechada** de por qué se hizo lo que se hizo. La frase **no gobierna ninguna ejecución** —el gate y las suites, que sí deciden verde o rojo, están corregidos—, así que se declara falsa aquí en vez de reescribir el registro. Si algún día `V84` hay que tocarla por una razón funcional, esta línea se arregla en el mismo viaje |
| **V84 · la escritura** | **Un segundo comentario de `V84` atribuye el cierre de la ESCRITURA sólo al trigger**: «Lo que se cierra es la ESCRITURA: `exigir_atributo_gobernado` exige `c.activo = true`». Por la API no es así — quien rechaza es `AtributosGobernados.definicionDe` → `CatalogoAtributoRepository.porClave`, y el trigger no se llega a ejecutar (medido por HTTP el 2026-08-26, evidencia de 5A §15). El trigger sí es quien rechaza el **INSERT directo** | `V84__la_ocupacion_y_los_servicios_con_vocabulario.sql`, viñeta «NO borra ni reinterpreta ni un solo valor escrito de esa clave» | Mismo motivo que la fila anterior: migración aplicada, no se edita. Es la **única** copia que queda en pie: las demás —código, documentación y el comentario del gate— están corregidas, tras rehacer el barrido el 2026-08-26 clasificando todas las ocurrencias en vez de la primera de cada archivo (evidencia de 5A §15) |
...
| **N20** | **Los campos nuevos del contrato-dato no tienen gate.** El lote de certificación añadió a `modelo-universal.js` los campos `retirado`, `retiradaPor`, `sustituidaPor` (y usa `exigencia`, `opciones`, `aplicaTodos`), y **`gate-modelo-universal.js` no menciona ninguno**: medido el 2026-08-25, 0 apariciones de los seis en el gate —barrido con control positivo, `clave` da 7 en ese mismo fichero—. El único consumidor de alguno es `motor-captura.js`, en `pasosDeAtributos` (`filter((a) => !a.retirado)`); las dos apariciones de `opciones` en el motor están las dos en `planBase` y son sus propias listas de paso, no el campo del catálogo. Hoy los tres campos nuevos son **ciertos**, pero cada uno contra una fuente distinta y **ninguno contra una columna homónima**: `catalogo_atributo` no tiene columna `retirada_por` ni `sustituida_por` (medido el 2026-08-26: sus 19 columnas no incluyen ninguna de las dos). Sólo `retirado` tiene contraparte —`catalogo_atributo.activo`—; `retiradaPor: "V84"` se contrasta con la migración que la retira, y `sustituidaPor` con la existencia de las dos claves sustitutas en el catálogo. Ciertos, sí, pero son **prosa inerte** dentro del fichero cuyo propósito declarado es «el contrato como dato ejecutable», y nada los compara con la base. Es el mismo mecanismo que dejó `servicios_disponibles` declarada viva | `docs/ai/modelo/gate-modelo-universal.js`, `docs/ai/modelo/modelo-universal.js` | **Distinto de N18**: aquélla es la **deriva** de contenido (97 claves en el Core, 22 declaradas); ésta es la **ausencia de gate** sobre los campos nuevos. Escribir esa comprobación exige antes decidir contra qué compara —el catálogo completo o el subconjunto—, que es justo la pregunta que N18 deja en CONTROL |
| **N21** | **La numeración del gate SQL es posicional y las citas a un número son frágiles.** `gate-modelo-universal.sql` declara `CREATE TEMP TABLE resultado (n serial, …)`: el número de cada comprobación es su **orden de inserción**, así que insertar una por encima desplaza todas las de abajo. Tres textos citan hoy «la comprobación 92» —§2.3 de este documento, la evidencia de 5A (§ del caso de conservación) y el javadoc de `OcupacionYServiciosIntegrationTest.sembrarLegadoAmbiguo`—, y la cita es correcta hoy (así salió en la corrida registrada: `controllocal_repositorios 92 5A CONTROL el predicado del legado…`), pero envejece sola y **en silencio**: nada falla cuando deja de serlo. **Esta fila ya lo demostró**: nació citando `OcupacionYServiciosIntegrationTest:916` y trece líneas de comentario añadidas cuarenta y ocho minutos después, el mismo día, la desplazaron a la 929 sin que nada avisara. Por eso ahora cita por nombre | `backend-spring/verificacion/gate-modelo-universal.sql` (`CREATE TEMP TABLE resultado`) y los tres textos citados | No se corrige en este lote: sustituir el número de comprobación por su **nombre** —`5A CONTROL el predicado del legado caza una traduccion sin linaje`— en los tres sitios es cambio de texto en un test además de en dos documentos, y la comprobación que ya se apoya en el nombre (`INFORME el control del legado imprime su universo`) demuestra que el nombre es la referencia estable. Alcance propio |
| **N22** · *decisión de CONTROL: no se cierra en 5A* | **Una clave `LISTA_MULTIPLE` retirada sigue admitiendo que le QUITEN opciones.** `escribirAlAlta` y `escribirEnEdicion` comprueban `esMultivalor()` y delegan en `escribirMultivalor` **antes** de llamar a `definicionDe`, y `escribirMultivalor` consulta el catálogo **sólo cuando el ancla no existe** (para crearla, vía `convertirMultivalor`). Con ancla ya existente hace `multivalores.save` y `multivalores.borrarDe` **sin mirar `catalogo_atributo` ni una vez**. La red de atrás cubre la mitad: `exigir_opcion_gobernada` exige `c.activo = true`, pero su trigger es `BEFORE INSERT OR UPDATE` —así que **caza el añadir y no ve el borrar**. Consecuencia: la fila «una clave retirada no admite valores nuevos — 400 por la API» tiene una excepción. En el **ALTA** se cumple (propiedad nueva ⇒ sin ancla ⇒ pasa por `definicionDe`); en la **EDICIÓN** de una `LISTA_MULTIPLE` con ancla existente, no | `controllocal-service/…/soporte/AtributosGobernados.java` (`escribirAlAlta`, `escribirEnEdicion`, `escribirMultivalor`), trigger `tg_opcion_gobernada` de `V72` | **Hoy no hay dato afectado, medido en `controllocal_dev` el 2026-08-26**: la única clave retirada es `servicios_disponibles`, que es `LISTA` (escalar, no pasa por esta rama), y las **cuatro** `LISTA_MULTIPLE` —`areas_comunes`, `cargas_gravamenes`, `equipamiento_incluido`, `vigilancia`— están **activas**; `atributo_propiedad_opcion` tiene **0 filas**. **En `controllocal_repositorios` sí es alcanzable hoy**: los fixtures dejan claves `zz_*` de tipo `LISTA_MULTIPLE` retiradas y con opciones escritas. Es residuo de pruebas en una base de pruebas —no dato de cartera— y se dice para que no se descubra como sorpresa. **Se dispara en `dev` el día que se retire una clave `LISTA_MULTIPLE`** que ya tenga ancla escrita: bastaría editar esa propiedad quitando una opción para que el borrado pasara sin rechazo. **Nada lo vigila**: ninguna prueba ni comprobación de gate cubre el caso. Cerrarlo es mover la consulta al catálogo al principio de `escribirMultivalor` —o llevar el trigger a `DELETE`—, y las dos son decisiones sobre el contrato de escritura del multivalor. **CONTROL decidió no cerrarlo en 5A** |
